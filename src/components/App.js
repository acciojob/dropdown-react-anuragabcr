...
// 			{data[state].city.map((city, index) => (
...
// 			<select id="landmark" value={landmarks} onChange={e => setLandmarks(e.target.value)}>
// 				{data[state].city[city].landmarks.map((landmarks, index) => (
...
function App() {
  ...
  const [cityData, setCityData] = useState(states[0].city);
  const [landmarkData, setLandmarkData] = useState(states[0].city[0].landmarks);
  const [state, setState] = useState(0);
  const [city, setCity] = useState(0);
  ...
  const changeState = (e) => {
    ...
    setLandmarkData(states[e.target.value].city[0].landmarks)
